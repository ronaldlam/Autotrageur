...
def get_arb_opportunities_by_orderbook(
    ...
    Args:
        trading_client1 (CCXTTrader): The trading client for exchange 1.
        trading_client2 (CCXTTrader): The trading client for exchange 2.
    ...
    try:
        ex1_market_buy = trading_client1.get_adjusted_market_price_from_orderbook(
            ex1_orderbook[ASKS])
    except trading_client.OrderbookException:
        ex1_market_buy = None
    try:
        ex1_market_sell = trading_client1.get_adjusted_market_price_from_orderbook(
            ex1_orderbook[BIDS])
    except trading_client.OrderbookException:
        ex1_market_sell = None
    try:
        ex2_market_buy = trading_client2.get_adjusted_market_price_from_orderbook(
            ex2_orderbook[ASKS])
    except trading_client.OrderbookException:
        ex2_market_buy = None
    try:
        ex2_market_sell = trading_client2.get_adjusted_market_price_from_orderbook(
            ex2_orderbook[BIDS])
    except trading_client.OrderbookException:
        ex2_market_sell = None

    logging.info("%s buy of %s, %s price: %s" %
                 (trading_client1.exchange_name, trading_client1.target_amount,
                  trading_client1.base, ex1_market_buy))
    logging.info("%s buy of %s, %s price: %s" %
                 (trading_client2.exchange_name, trading_client2.target_amount,
    ...
    logging.info("Ex2 (%s) sell Ex1 (%s) buy spread: (%s)" %
                 (trading_client2.exchange_name,
                  trading_client1.exchange_name,
...
def execute_arbitrage(opportunity):
    ...
    try:
        ...
        # TODO: Need something for dry run here.
